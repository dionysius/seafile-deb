...
After installation, configure the server by editing `/etc/seafile/seafile.env` (secrets, database, cache, hostname, feature toggles) and `/etc/seafile/seafile.conf`. A random `JWT_PRIVATE_KEY` is generated on first install. Restart afterwards with `systemctl restart seafile seafile-fileserver seafile-notification seahub`.
